  ...
  group("Run Command Tests", () {
    test('Should create a tmp project', () async {
      if (!stdin.hasTerminal) {
        print("Test skipped, terminal unavailable.");
        return;
      }

      /// Arrange (Creating a new gazelle project)
      String path = "tmp/run/test_project";
      ...
      final tmpDir = Directory("$path/.tmp");
      ...
      await Directory("tmp/run").delete(recursive: true);
    });
    ...
    test('Should run for 10 seconds', () async {
      ...
      final tmpDir = Directory("$path/.tmp").absolute.path;
